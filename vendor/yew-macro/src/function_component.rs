...
fn parse_component_function(input: ParseStream) -> syn::Result<ItemFn> {
...
fn validate_component_signature(sig: &syn::Signature) -> syn::Result<()> {
...
fn component_return_type(sig: &syn::Signature) -> syn::Result<Box<Type>> {
    match &sig.output {
...
fn component_props_type(arg: &FnArg) -> syn::Result<Box<Type>> {
    ...
    let Type::Reference(reference) = &*arg.ty else {
        let msg = format!(
            "expected a reference to a `Properties` type (try: `&{}`)",
            arg.ty.to_token_stream()
        );
        return Err(syn::Error::new_spanned(&arg.ty, msg));
    };
...
fn component_props(inputs: Punctuated<FnArg, Comma>) -> syn::Result<(FnArg, Box<Type>)> {
    let mut inputs = inputs.into_iter();
    let arg = inputs
        .next()
        .unwrap_or_else(|| syn::parse_quote! { _: &() });
    let props_type = component_props_type(&arg)?;
    if inputs.len() > 0 {
        let params: TokenStream = inputs.map(|item| item.to_token_stream()).collect();
        return Err(syn::Error::new_spanned(
            params,
...
impl Parse for FunctionComponent {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        let func = parse_component_function(input)?;
        ...
        validate_component_signature(&sig)?;
        let return_type = component_return_type(&sig)?;
        let (arg, props_type) = component_props(sig.inputs)?;
