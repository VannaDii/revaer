...
if [[ "${expected_bytes}" != "12" ]]; then
  printf 'test-download-integrity: fixture_size returned unexpected byte count: %s\n' \
    "${expected_bytes}" >&2
  exit 1
fi

fixture_size_failure_status=0
if (
  wc() {
    return 37
  }
  fixture_size "${temporary_root}/expected" >/dev/null
); then
  printf 'test-download-integrity: fixture_size masked a failing byte-count pipeline\n' >&2
  exit 1
else
  fixture_size_failure_status="$?"
fi
if ((fixture_size_failure_status != 37)); then
  printf 'test-download-integrity: fixture_size returned %s for pipeline status 37\n' \
    "${fixture_size_failure_status}" >&2
  exit 1
fi
...
printf 'test-download-integrity: size status, corruption, oversize, mismatch, timeout, mirror fallback, and base64 decoding passed\n'
