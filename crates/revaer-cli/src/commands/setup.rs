...
mod tests {
    ...
    const RESUME_FILE_WRITE_TIMEOUT: Duration = Duration::from_secs(30);

    async fn read_resume_file_after_write(path: &Path) -> Result<String> {
        timeout(RESUME_FILE_WRITE_TIMEOUT, async {
            loop {
        ...
        .unwrap_or_else(|_| {
            Err(anyhow!(
                "resume file was not written after {}s",
                RESUME_FILE_WRITE_TIMEOUT.as_secs()
            ))
        })
    }
