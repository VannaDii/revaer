...
- Dependency rationale:
  - No new third-party package was added. `chrono` moved from runtime dev-dependencies to dependencies because the runtime facade exposes an existing workspace timestamp type; the app now directly uses the existing workspace `rustix` package for descriptor-relative, no-follow fingerprint traversal.
- Stale-policy check:
