...
- Remove an empty checkout shallow marker before Sonar analysis and reject any nonempty marker so the scanner cannot discard SCM blame after Git reports a complete checkout.
...
- Design notes:
  ...
  - The post-scan verifier retries the complete published-result snapshot for a bounded 30-second window so Sonar API indexing can converge after the compute task succeeds. Every retry reasserts positive coverage and lines-to-cover, a passing gate with no ignored conditions, zero unresolved new-code issues, and zero unreviewed new-code hotspots.
