...
The fixture-backed test suite exercises stream-copy/remux behavior, sidecar
subtitle artifact operations, and real audio/video transcoding through the
production media planner. Transcode cases assert the planned operation kind and
then probe the materialized output to verify the resulting video and audio
codecs.
...
checks, production pipeline materialization actions, sidecar artifact actions,
and audio/video transcode counts in the Markdown report.
