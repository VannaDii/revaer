...
fn collect_argument_lifetimes(sig: &mut Signature) -> Vec<Lifetime> {
...
fn add_lifetime_hook_bound(
    param: &mut LifetimeParam,
    generics: &mut syn::Generics,
    hook_lifetime: &Lifetime,
) {
    let predicate = generics
        ...
        .find_map(|predicate| match predicate {
            WherePredicate::Lifetime(predicate) if predicate.lifetime == param.lifetime => {
                Some(predicate)
            }
            _ => None,
        });
...
fn add_type_hook_bound(
    ...
    let predicate = generics
        ...
        .flat_map(|clause| &mut clause.predicates)
        .find_map(|predicate| match predicate {
            WherePredicate::Type(predicate) if type_is_generic(&predicate.bounded_ty, param) => {
                Some(predicate)
            }
            _ => None,
        });
    let bound = TypeParamBound::Lifetime(hook_lifetime.clone());
    if let Some(predicate) = predicate {
        predicate.bounds.push(bound);
    } else {
        param.colon_token = Some(param.colon_token.unwrap_or_default());
...
fn add_hook_bound(
    ...
    match param {
        GenericParam::Lifetime(param) => {
            add_lifetime_hook_bound(param, generics, hook_lifetime);
        }
        GenericParam::Type(param) => add_type_hook_bound(param, generics, hook_lifetime),
...
impl HookSignature {
    ...
    pub fn rewrite(sig: &Signature) -> syn::Result<Self> {
        ...
        for type_param in params.iter_mut().skip(1) {
            add_hook_bound(type_param, generics, &hook_lifetime);
        }
