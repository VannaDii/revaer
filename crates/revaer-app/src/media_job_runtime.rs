...
fn container_chapters_from_snapshot(
    ...
    for row in rows {
        ...
        let chapter_index = chapters.iter().position(|chapter: &ContainerChapterEntry| {
        ...
        match (row.metadata_key, row.metadata_value) {
            (Some(key), Some(value)) => {
                append_chapter_metadata(&mut chapters[index], &key, &value)?;
            }
            (None, None) => {}
            _ => {
                return Err(MediaJobRuntimeError::InvalidDesiredGraph(
...
mod tests {
    ...
    impl InspectAdapter for AttachmentStripInspector {
        fn inspect(&self, source_path: &str) -> Result<MediaGraph, InspectError> {
            let codec = match fs::read(source_path) {
                Ok(bytes) if bytes.as_slice() == b"output" => "hevc",
                Ok(_) | Err(_) if source_path.contains("/workspace/") => "hevc",
                Ok(_) | Err(_) => "h264",
            };
            let mut graph = video_graph(source_path, codec);
            if codec == "h264" {
