...
use revaer_media_core::target::{
    ...
    ImageSubtitleAction, LanguageToken, SidecarSubtitleInput, SubtitlePlacement, TargetStream,
...
impl MediaJobRuntime {
    ...
    async fn handle_preflight_ready(
        ...
        if planned_job_is_noop(&report) {
            self.complete_noop_job(job, verification_context, shutdown)
                .await?;
            return Ok(TerminalWorkspaceState::Completed);
        }
...
fn container_chapters_from_snapshot(
    ...
    for row in rows {
        ...
        match (row.metadata_key, row.metadata_value) {
            (Some(key), Some(value)) => {
                append_chapter_metadata(chapter, &key, &value)?;
            }
            (None, None) => {}
            _ => {
                return Err(MediaJobRuntimeError::InvalidDesiredGraph(
    ...
    for chapter in &mut chapters {
        if previous_end.is_some_and(|end| chapter.start_millis < end) {
            return Err(MediaJobRuntimeError::InvalidDesiredGraph(
                "media_job_desired_container_chapter_overlap",
            ));
        }
...
fn append_chapter_metadata(
    ...
    if chapter.metadata.iter().any(|entry| entry.key == key) {
        return Err(MediaJobRuntimeError::InvalidDesiredGraph(
            "media_job_desired_container_chapter_metadata_duplicate",
        ));
    }
...
fn compile_runtime_desired_target(
    ...
    let Some(snapshot) = desired_target else {
        return Ok(CompiledDesiredTarget {
            graph: DesiredGraph {
                output_path: output_path.to_string(),
                container_format: None,
...
mod tests {
    ...
    use revaer_media_core::plan::{OperationKind, PlannedOperation};
    use revaer_media_core::target::{
        DesiredSidecarOutput, DesiredTarget, Hdr10ColorVolume, SidecarOutputSource, TargetStream,
        UnmatchedStreamPolicies, UnmatchedStreamPolicy,
    };
    use revaer_media_runtime::execute::{
