ALTER TABLE media_desired_target_audio_stream
    ADD COLUMN audio_bitrate_bps INT,
    ADD COLUMN audio_sample_rate_hz INT,
    ADD CONSTRAINT media_desired_target_audio_bitrate_positive CHECK (
        audio_bitrate_bps IS NULL OR audio_bitrate_bps > 0
    ),
    ADD CONSTRAINT media_desired_target_audio_sample_rate_positive CHECK (
...
CREATE FUNCTION media_desired_target_stream_append_v4(
    ...
    IF stream_kind_value = 'audio' THEN
        ...
        IF audio_sample_rate_hz_input IS NOT NULL AND audio_sample_rate_hz_input <= 0 THEN
            RAISE EXCEPTION 'audio sample rate must be positive'
                USING ERRCODE = media_app_error_code_v1(), DETAIL = 'media_desired_target_audio_shape_invalid';
        END IF;
    ELSIF audio_bitrate_bps_input IS NOT NULL OR audio_sample_rate_hz_input IS NOT NULL THEN
...
CREATE OR REPLACE FUNCTION media_desired_target_stream_append_v3(
    media_desired_target_profile_public_id_input UUID,
    stream_key_input TEXT,
    stream_kind_input TEXT,
    semantic_role_input TEXT,
...
CREATE FUNCTION media_desired_target_stream_list_v4(
    media_desired_target_profile_public_id_input UUID
)
RETURNS TABLE (
    stream_key TEXT,
...
CREATE FUNCTION media_job_desired_target_audio_constraints_snapshot_v1()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
CREATE TRIGGER trg_media_job_desired_target_audio_constraints_snapshot
BEFORE INSERT ON media_job_desired_target_stream
FOR EACH ROW
EXECUTE FUNCTION media_job_desired_target_audio_constraints_snapshot_v1();

CREATE FUNCTION media_job_desired_target_stream_list_v4(
