...
- ExifTool is the only approved first-release interpreted-utility exception.
  It is included because no compiled open-source replacement provides equivalent
  robust cross-format metadata coverage for Revaer media workflows.
- The ExifTool exception must be implemented as a bounded runtime adapter:
  no arbitrary operator-supplied command fragments, no unrestricted write mode,
  no direct source-path mutation, no source-adjacent temporary files, and no
  execution outside the managed workspace.
- The ExifTool exception must include license text, attribution, upstream
  source URL, exact version, checksum, source-compliance entry, and security
  review evidence in every release artifact set.
...
| Bento4 CLI (`mp4dump`, `mp4info`, `mp4edit`, `mp4tag`, `mp4extract`, `mp4fragment`, `mp4compact`) | [bento4.com](https://www.bento4.com/downloads/)                                                                                              | Secondary MP4 atom inspection, structural validation, metadata editing, extraction, and packaging diagnostics.                                                                                                           | 1.6.0-641                                                                                  | Complements GPAC, FFmpeg, and ExifTool for verification and MP4 diagnostics; Revaer verification may use it for strict MP4 profiles.                                                                                                                                                                        |
...
| ExifTool                                                                                          | [exiftool.org](https://exiftool.org/)                                                                                                        | Robust cross-format metadata inspection and bounded metadata rewrite support for container, stream, artwork, attachment, and tag edge cases that FFmpeg, MediaInfo, MKVToolNix, GPAC, and Bento4 do not cover uniformly. | 13.55 production; 13.58 latest development                                                 | First-release source-form utility exception. Complements FFmpeg, MediaInfo, MKVToolNix, GPAC, and Bento4; Revaer must run it only through a bounded adapter inside the managed workspace and must not use it to create source-adjacent clutter.                                                             |
...
| Source-form utility exception               | ExifTool only                                                                                                                      | Ship ExifTool as the sole approved interpreted utility because it materially improves metadata coverage. Include exact source/version evidence, license notices, adapter allowlist, security review, and workspace-only execution constraints. Reject every other source-form runtime utility unless a later ADR grants a narrower exception. | ExifTool exception ADR, adapter tests, inventory entry, notices, source-compliance bundle                            |
...
- Fail the build if the final image contains source archives, source checkouts,
  ...
  builder-only tools.
- Fail the build if any interpreted/source-form runtime utility other than
  ExifTool is present.
- Fail the build if ExifTool is present without an exception ADR, exact version
  and checksum, license and attribution files, source-compliance entry, adapter
  allowlist, managed-workspace-only execution tests, and security review
  evidence.
...
9. Docker image codec, toolchain, and license-compliance coverage
    ...
    - Exclude nonfree, proprietary, unapproved source-form, or
      license-incompatible components from the default image.
    - Include ExifTool as the only approved first-release source-form runtime
      utility exception, with bounded adapter tests and release-compliance
      evidence.
    ...
    - Generate exact source-compliance bundle and source-offer text for the image
      digest.
    - Add CI/release gates for required tools, builder-artifact-free final
      runtime images, nonfree configure flags, unapproved source-form utilities,
      license detection, notices, source bundle completeness, Revaer-to-GPL
      direct linkage, and publication metadata.
...
- Final runtime image contains no source archives, source checkouts, VCS
  ...
  caches.
- Final runtime image contains no interpreted/source-form utility except the
  approved ExifTool exception.
- ExifTool is present only with exact version/checksum evidence, license and
  attribution files, source-compliance bundle entry, adapter allowlist,
  managed-workspace-only execution tests, and security review evidence.
