ALTER TABLE media_desired_target_container
    ADD COLUMN container_attachment_policy TEXT NOT NULL DEFAULT 'preserve',
    ADD CONSTRAINT media_desired_target_container_attachment_policy_known CHECK (
        container_attachment_policy IN ('preserve', 'strip')
    );

ALTER TABLE media_job
    ADD COLUMN intent_desired_container_attachment_policy TEXT;

UPDATE media_job
   SET intent_desired_container_attachment_policy = 'preserve'
 WHERE intent_desired_target_profile_id IS NOT NULL
   AND intent_desired_container_attachment_policy IS NULL;
...
ALTER TABLE media_job
    ...
    ADD CONSTRAINT media_job_intent_desired_target_complete CHECK (
        ...
        (intent_desired_target_profile_id IS NOT NULL
            ...
            AND intent_desired_container_attachment_policy IN ('preserve', 'strip'))
    );

CREATE FUNCTION media_desired_target_create_v4(
    ...
    container_attachment_policy_input TEXT
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE FUNCTION media_desired_target_list_v4()
RETURNS TABLE (
...
CREATE FUNCTION media_job_worker_claim_next_v5()
RETURNS TABLE (
