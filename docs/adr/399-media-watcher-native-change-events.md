...
- Decision:
  ...
  - Drive runtime queueing, fingerprint de-duplication, and readiness-gate tests with deterministic injected watcher events instead of relying on host watcher delivery timing.
- Consequences:
  - Positive outcomes:
    ...
    - The runtime tests now prove durable file versions are queued exactly once without depending on native event latency in CI.
  - Risks or trade-offs:
...
- Test coverage summary:
  ...
  - `just policy`
  - `cargo --config 'build.rustflags=["-Dwarnings"]' test -p revaer-app --no-default-features --lib --bins --tests media_discovery_runtime::tests::watcher_only_profile_enqueues_each_durable_file_version_exactly_once`
