...
- CI-generated ignored outputs must stay in the coverage-producing job. The Sonar scan job must start from a clean checkout, fetch and verify the actual pull-request base branch for PR analysis, install `libtorrent-rasterbar-dev` for the system headers referenced by the native compile database, and download only `coverage/lcov.info`, `coverage/compile_commands.json`, `coverage/cxxbridge/include/rust/cxx.h`, and `coverage/cxxbridge/include/revaer-torrent-libt/src/ffi/bridge.rs.h`; do not use SCM ignore behavior as an analysis escape hatch.
- If Sonar noise appears to come from first-party authored code, fix the code. If it appears to come from generated, vendored, binary, or transient files, prefer deleting, regenerating, replacing the artifact, or using an exact matching analyzer configuration over adjusting `sonar-project.properties`; changing scan criteria still requires explicit operator consent recorded in the ADR.
