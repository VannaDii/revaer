...
fn peek_identifier_html_type(input: ParseStream) -> Option<HtmlType> {
    ...
    let component = name
        ...
        || input.peek(Token![::])
        || is_ide_completion() && name.chars().any(|character| character.is_ascii_uppercase());
    Some(if component {
        HtmlType::Component
    } else {
        HtmlType::Element
...
fn peek_tag_html_type(input: ParseStream) -> Option<HtmlType> {
    ...
    if input.peek(Token![>]) {
        ...
        peek_identifier_html_type(input)
    } else {
        None
    }
}
...
impl HtmlTree {
    ...
    fn peek_html_type(input: ParseStream) -> Option<HtmlType> {
        ...
        if input.is_empty() {
            return Some(HtmlType::Empty);
        }
        if HtmlBlock::peek(cursor).is_some() {
            return Some(HtmlType::Block);
        }
        if HtmlIf::peek(cursor).is_some() {
            return Some(HtmlType::If);
        }
        if HtmlFor::peek(cursor).is_some() {
            return Some(HtmlType::For);
        }
        input.peek(Token![<]).then(|| peek_tag_html_type(&input))?
    }
}
...
impl HtmlChildrenTree {
    ...
    pub fn fully_keyed(&self) -> Option<bool> {
        for child in self.0.iter() {
            if let Err(result) = keyed_child(child) {
                return result;
            }
        }
        Some(true)
...
fn keyed_child(child: &HtmlTree) -> Result<(), Option<bool>> {
    match child {
        HtmlTree::Block(block) => Err(match &block.content {
            BlockContent::Node(node) => matches!(&**node, HtmlNode::Literal(_)).then_some(false),
            _ => None,
        }),
        HtmlTree::Component(component) if component.props.props.special.key.is_none() => {
            Err(Some(false))
        }
        HtmlTree::List(list) if list.open.props.key.is_none() => Err(Some(false)),
        HtmlTree::Element(element) if element.props.special.key.is_none() => Err(Some(false)),
        HtmlTree::If(_) | HtmlTree::For(_) | HtmlTree::Empty => Err(Some(false)),
        _ => Ok(()),
    }
}
