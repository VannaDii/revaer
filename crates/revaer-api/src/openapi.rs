...
mod tests {
    ...
    #[test]
    fn desired_stream_schema_exports_audio_policy_fields() -> Result<(), Box<dyn std::error::Error>>
    {
        let document = openapi_document();
        let properties = document
            .pointer("/components/schemas/MediaDesiredTargetStream/properties")
            .and_then(Value::as_object)
            .ok_or_else(|| io::Error::other("expected desired stream properties"))?;

        for field in ["audio_loudness_profile", "audio_dynamic_range"] {
            assert!(
                properties.contains_key(field),
                "missing {field} schema field"
            );
        }
        Ok(())
    }
