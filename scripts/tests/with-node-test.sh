...
mkdir -p "${nvm_dir}" "${nvm_bin}" "${fallback_bin}" "${wrong_bin}"
...
cat > "${wrong_bin}/node" <<'NODE'
#!/usr/bin/env bash
printf 'v24.18.0\n'
NODE
chmod +x "${wrong_bin}/node"

explicit_output="$(
  NVM_DIR="${nvm_dir}" \
  MOCK_NVM_BIN="${nvm_bin}" \
  MOCK_NVM_LOG="${nvm_log}" \
  REVAER_NODE_VERSION="24.19.0" \
    bash "${repo_root}/scripts/with-node.sh" node --version
)"
[[ "${explicit_output}" == "v24.19.0" ]]
grep -Fxq 'use --silent 24.19.0' "${nvm_log}"
...
if PATH="${fallback_bin}:${PATH}" \
  ...
  MOCK_NVM_LOG="${nvm_log}" \
  MOCK_NVM_FAIL=1 \
    bash "${repo_root}/scripts/with-node.sh" node --version >/dev/null 2>&1; then
  echo "with-node accepted an unavailable pinned NVM version" >&2
  exit 1
fi
...
[[ "${path_output}" == "v24.19.0" ]]

if PATH="${wrong_bin}:${PATH}" NVM_DIR="${test_root}/missing-nvm" \
  bash "${repo_root}/scripts/with-node.sh" node --version >/dev/null 2>&1; then
  echo "with-node accepted a mismatched PATH Node version" >&2
  exit 1
fi
...
if [[ "${missing_lts_status}" -ne 3 ]]; then
    echo "with-node returned ${missing_lts_status} for a mismatched Node version" >&2
  exit 1
fi
...
printf '%s\n' "NVM-aware Node wrapper tests passed"
