...
impl MediaStore {
    ...
    pub async fn append_job_operation(
        ...
        append_media_job_operation(&self.pool, claim_generation, input).await
    }
    ...
    pub async fn append_job_plan_reason(
        ...
        append_media_job_plan_reason(&self.pool, claim_generation, input).await
    }
...
mod tests {
    ...
    async fn claim_job(store: &MediaStore, job_id: Uuid) -> anyhow::Result<i64> {
        let claimed = store
            .claim_next_job()
            .await?
            .ok_or_else(|| anyhow::anyhow!("expected queued media job to be claimable"))?;
        anyhow::ensure!(
            claimed.media_job_public_id == job_id,
            "claimed an unexpected media job"
        );
        Ok(claimed.claim_generation)
    }
    ...
    async fn append_and_assert_job_records(
        store: &MediaStore,
        profile_id: Uuid,
        job_id: Uuid,
        claim_generation: i64,
        ...
        let jobs = store.list_jobs(profile_id, Some("running")).await?;
    ...
    async fn append_cleanup_diagnostics(
        store: &MediaStore,
        job_id: Uuid,
        claim_generation: i64,
    ) -> anyhow::Result<()> {
    ...
    async fn assert_job_and_worker_errors(store: &MediaStore, actor_id: Uuid, profile_id: Uuid) {
        ...
        assert!(store.complete_job(job_id, 0, 0).await.is_err());
        assert!(
            store
                .mark_job_status(job_id, 0, "running", Some("transcoding"))
                .await
                .is_err()
        );
    }
