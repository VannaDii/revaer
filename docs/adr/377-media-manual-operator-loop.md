...
- Decision:
  ...
  - Add `media_manual_job_create_v1` for the original manual loop and `media_manual_job_create_v2` after the source-identity fence so manual jobs persist device/inode identity and change time alongside size, modification time, and aggregate SHA-256.
...
- Design notes:
  ...
  - The data layer adds a manual creation procedure instead of weakening discovery fingerprint deduplication. Migration 0185 advances that procedure to the complete source-identity contract introduced by migration 0184.
...
- Test coverage summary:
  ...
  - `cargo test -p revaer-app --all-features manual_job -- --nocapture`
  - `cargo test -p revaer-api --all-features create_media_job -- --nocapture`
  - `cargo test -p revaer-data manual_job_creation_refreshes_identity_and_allows_repeated_runs -- --nocapture` validates repeated manual runs against the complete source identity snapshot when a test database is available.
...
- Risk & rollback plan:
  - Roll back migrations 0177 and 0185 and the API/app/UI wiring if manual job creation bypasses operator safeguards.
