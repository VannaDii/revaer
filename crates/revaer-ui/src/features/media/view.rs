...
use crate::features::media::api::{
    ...
    patch_profile, preview_discovery, refresh_capability, validate_yaml,
};
...
use crate::models::{
    ...
    MediaJobVerificationCheckResponse, MediaJobViolationResponse, MediaProfilePatchRequest,
        ...
        Callback::from(
            move |(media_profile_public_id, default_source_path): (uuid::Uuid, String)| {
                ...
                if requested_source_path.trim().is_empty() {
                    on_error_toast.emit("Discovery source path is required".to_string());
                    return;
                }
