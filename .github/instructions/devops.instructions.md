...
- Asset verification must run through `just check-assets`, which invokes the canonical synchronizer and compares `crates/revaer-ui/static/nexus/**` from the repository root; a cwd-relative comparison that misses served assets is forbidden. Release validation must also confirm that referenced `/static/...` icon, logo, and DataTables URLs exist in the Trunk release output.
