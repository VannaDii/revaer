...
- `pr.yml` must run the media conversion fixture gate through `just download-test-fixtures`, `just generate-test-fixtures`, `just verify-test-fixtures`, and `just test-media-conversion`, with ignored fixture binary directories restored from a cache keyed by the fixture manifest, scripts, and media tool versions.
