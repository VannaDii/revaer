...
- Workflow guardrails must reject every job `needs` reference that does not name a job in the same workflow. A workflow that GitHub cannot schedule is a failed required gate, not an absent or skipped check.
...
- Every workflow job that declares a Postgres service, including API E2E, must construct `REVAER_TEST_DATABASE_URL`, `DATABASE_URL`, and any E2E admin URL from that service's exact run-derived user, password, and database. Repository variables must not override or drift from an in-job service credential.
