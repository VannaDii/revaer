...
- `pr.yml` must run Cargo supply-chain checks through one combined `supply-chain` job that restores version-keyed installed-tool caches, restores the Cargo advisory database cache, installs `cargo-audit`, `cargo-deny`, and `cargo-udeps` through the pinned prebuilt installer action, and then calls `just audit`, `just deny`, and `just udeps`. Keep downstream jobs gated on `supply-chain`; do not split those checks back into separate setup-heavy jobs unless a new ADR records the timing evidence.
...
- Media runtime image publication must remain disabled unless the reusable image workflow receives and validates an explicit final-image compliance bundle generated for the exact image build. `build-images.yml` must run `scripts/validate-final-image-compliance-bundle.sh` before any image login, push, manifest, signing, or Helm publication step. Do not pass `final_image_compliance_bundle` from `ci.yml` until release automation creates that final-image evidence before the first push.
...
- `pr.yml` must run the media conversion fixture gate through `just download-test-fixtures`, `just generate-test-fixtures`, `just verify-test-fixtures`, and `just test-media-conversion`, with ignored fixture binary directories restored from a cache keyed by the fixture manifest, scripts, and media tool versions. The gate must publish the generated `target/media-conversion-report.md` into the GitHub job summary and upload it as a workflow artifact so the media actions and outcomes are reviewable from one report.
...
- `just db-start` must honor an already reachable, explicitly supplied non-Docker `DATABASE_URL` before running Docker lifecycle commands, while preserving Docker container management for the default localhost development endpoint.
