ALTER TABLE media_desired_target_stream
    ADD COLUMN subtitle_placement TEXT,
    ADD COLUMN image_subtitle_action TEXT;

UPDATE media_desired_target_stream
   SET subtitle_placement = 'embedded',
       image_subtitle_action = 'fail'
 WHERE stream_kind = 'subtitle';

ALTER TABLE media_desired_target_stream
    ADD CONSTRAINT media_desired_target_stream_subtitle_shape CHECK (
        (stream_kind = 'subtitle'
            AND subtitle_placement IN ('embedded', 'sidecar', 'both', 'none')
            AND image_subtitle_action IN ('preserve', 'remove', 'fail'))
        OR
        (stream_kind <> 'subtitle'
...
UPDATE media_job_desired_target_stream
   SET subtitle_placement = 'embedded',
       image_subtitle_action = 'fail'
 WHERE stream_kind = 'subtitle';

ALTER TABLE media_job_desired_target_stream
    ADD CONSTRAINT media_job_desired_target_stream_subtitle_shape CHECK (
        (stream_kind = 'subtitle'
            AND subtitle_placement IN ('embedded', 'sidecar', 'both', 'none')
            AND image_subtitle_action IN ('preserve', 'remove', 'fail'))
        OR
...
CREATE FUNCTION media_desired_target_stream_append_v2(
...
CREATE FUNCTION media_desired_target_stream_list_v2(
    media_desired_target_profile_public_id_input UUID
)
RETURNS TABLE (
    stream_key TEXT,
...
CREATE FUNCTION media_job_desired_target_stream_list_v2(
...
BEGIN
    ...
    IF desired_target_row.media_desired_target_profile_id IS NOT NULL THEN
        INSERT INTO media_job_desired_target_stream (
            ...
            default_disposition, forced_disposition, subtitle_placement,
            image_subtitle_action
        )
