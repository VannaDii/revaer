...
impl<T> Shared<T> {
    ...
    fn remove_pending_sender(&self, hook: &Arc<Hook<T, SyncSignal>>) {
        let erased_hook: Arc<Hook<T, dyn signal::Signal>> = hook.clone();
        wait_lock(&self.chan)
            .sending
            .as_mut()
            .unwrap()
            .1
            .retain(|sender| sender.signal().as_ptr() != erased_hook.signal().as_ptr());
    }

    fn finish_deadline_send(
        &self,
        hook: Arc<Hook<T, SyncSignal>>,
        timed_out: bool,
    ) -> Result<(), TrySendTimeoutError<T>> {
        if timed_out {
            self.remove_pending_sender(&hook);
        }

        match hook.try_take() {
            Some(msg) if self.is_disconnected() => Err(TrySendTimeoutError::Disconnected(msg)),
            Some(msg) => Err(TrySendTimeoutError::Timeout(msg)),
            None => Ok(()),
        }
    }
    ...
    fn wait_sync_send(
        ...
        if let Some(deadline) = deadline {
            hook.wait_deadline_send(&self.disconnected, deadline)
                .or_else(|timed_out| self.finish_deadline_send(hook, timed_out))
        } else {
            hook.wait_send(&self.disconnected);
            hook.try_take()
                .map_or(Ok(()), |msg| Err(TrySendTimeoutError::Disconnected(msg)))
        }
    }
    ...
    fn send_sync(
        ...
        self.send(
            ...
            // do_block
            |hook| self.wait_sync_send(hook, block.flatten()),
        )
    }
    ...
    fn remove_pending_receiver(&self, hook: &Arc<Hook<T, SyncSignal>>) {
        let erased_hook: Arc<Hook<T, dyn Signal>> = hook.clone();
        wait_lock(&self.chan)
            .waiting
            .retain(|receiver| receiver.signal().as_ptr() != erased_hook.signal().as_ptr());
    }

    fn finish_deadline_recv(
        &self,
        hook: Arc<Hook<T, SyncSignal>>,
        timed_out: bool,
    ) -> Result<T, TryRecvTimeoutError> {
        if timed_out {
            self.remove_pending_receiver(&hook);
        }
        ...
        match wait_lock(&self.chan).queue.pop_front() {
            Some(msg) => Ok(msg),
            None if disconnected => Err(TryRecvTimeoutError::Disconnected),
    ...
    fn wait_sync_recv(
        ...
        if let Some(deadline) = deadline {
            hook.wait_deadline_recv(&self.disconnected, deadline)
                .or_else(|timed_out| self.finish_deadline_recv(hook, timed_out))
        } else {
    ...
    fn recv_sync(&self, block: Option<Option<Instant>>) -> Result<T, TryRecvTimeoutError> {
        self.recv(
            ...
            // do_block
            |hook| self.wait_sync_recv(hook, block.flatten()),
        )
    }
