...
- Treat `sonar-project.properties` as the versioned source of truth for Sonar analysis scope. Coverage exclusions must remain explicitly empty so Sonar imports the Rust LCOV and native LLVM coverage generated by `just cov` instead of publishing zero coverage.
- Sonar-running checkout steps must retain full history so pull-request new-code attribution and SCM blame use the reviewed base rather than a shallow checkout.
...
- `just cov` must pass its resolved test database URL to both database startup and the coverage process in the same recipe shell so database-backed tests cannot silently lose their configured endpoint. It must instrument linked first-party C/C++ through a Clang version compatible with rustc's LLVM tools, retain Rust-only package thresholds, and emit both Rust LCOV and native LLVM reports for Sonar. Disposable test databases must be dropped with forced session cleanup, and local-only host fallback must remain bounded to `localhost` and `127.0.0.1` inputs.
