...
fn map_media_error(
    ...
    let mut api_error = match err.kind() {
        ...
        MediaServiceErrorKind::Unavailable => ApiError::service_unavailable(detail),
    };
...
mod tests {
    ...
    #[tokio::test]
    async fn media_configuration_reads_report_unavailable_default_facade() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;

        let targets = list_media_compatibility_targets(State(state.clone()))
            .await
            .expect_err("default media facade must fail closed");
        let desired_targets = list_media_desired_targets(State(state.clone()))
            .await
            .expect_err("default media facade must fail closed");
        let policies = list_media_policies(State(state.clone()))
            .await
            .expect_err("default media facade must fail closed");
        let retention = media_job_retention(State(state))
            .await
            .expect_err("default media facade must fail closed");

        for error in [targets, desired_targets, policies, retention] {
            assert_eq!(
                error.into_response().status(),
                StatusCode::SERVICE_UNAVAILABLE
            );
    ...
    #[tokio::test]
    async fn create_media_job_maps_noop_to_service_unavailable() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
    ...
    #[tokio::test]
    async fn get_media_profile_reports_unavailable_default_facade() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
        ...
        let body = to_bytes(response.into_body(), 64 * 1024).await?;
        let problem: ProblemDetails = serde_json::from_slice(&body)?;
        assert_eq!(problem.detail.as_deref(), Some(MEDIA_PROFILE_LIST_FAILED));
        Ok(())
    }
    ...
    #[tokio::test]
    async fn list_media_jobs_normalizes_before_reporting_unavailable() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
        let query = MediaJobsQuery {
            media_profile_public_id: Some(Uuid::new_v4()),
        ...
        let error = list_media_jobs(State(state), Query(query))
            .await
            .expect_err("default media facade must fail closed");
        assert_eq!(
            error.into_response().status(),
    ...
    #[tokio::test]
    async fn get_media_job_reports_unavailable_default_facade() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
    ...
    #[tokio::test]
    async fn cancel_media_job_maps_noop_to_service_unavailable() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
    ...
    #[tokio::test]
    async fn retry_media_job_maps_noop_to_service_unavailable() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
    ...
    #[tokio::test]
    async fn list_media_job_violations_reports_unavailable_default_facade() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
        let error = list_media_job_violations(State(state), Path(Uuid::new_v4()))
            .await
            .expect_err("default media facade must fail closed");
        assert_eq!(
            error.into_response().status(),
    ...
    #[tokio::test]
    async fn list_media_job_plan_reasons_reports_unavailable_default_facade() -> anyhow::Result<()>
    {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
        let error = list_media_job_plan_reasons(State(state), Path(Uuid::new_v4()))
            .await
            .expect_err("default media facade must fail closed");
        assert_eq!(
            error.into_response().status(),
    ...
    #[tokio::test]
    async fn list_media_job_verification_checks_reports_unavailable_default_facade()
    -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
        let error = list_media_job_verification_checks(State(state), Path(Uuid::new_v4()))
            .await
            .expect_err("default media facade must fail closed");
        assert_eq!(
            error.into_response().status(),
    ...
    #[tokio::test]
    async fn list_media_job_artifacts_reports_unavailable_default_facade() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
        let error = list_media_job_artifacts(State(state), Path(Uuid::new_v4()))
            .await
            .expect_err("default media facade must fail closed");
        assert_eq!(
            error.into_response().status(),
    ...
    #[tokio::test]
    async fn list_media_job_compact_audits_reports_unavailable_default_facade() -> anyhow::Result<()>
    {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
        let error = list_media_job_compact_audits(State(state), Path(Uuid::new_v4()))
            .await
            .expect_err("default media facade must fail closed");
        assert_eq!(
            error.into_response().status(),
    ...
    #[tokio::test]
    async fn refresh_media_capability_maps_noop_to_service_unavailable() -> anyhow::Result<()> {
        let state = indexer_test_state(Arc::new(RecordingIndexers::default()))?;
    ...
    fn map_media_error_maps_unavailable_service() {
        ...
        let response = map_media_error("media_profile_list", MEDIA_PROFILE_LIST_FAILED, &error)
            .into_response();
