...
- Rust CLI tools invoked by required recipes must be installed at an exact reviewed version with the published lockfile. Floating or unlocked `cargo install` commands are forbidden because registry resolution drift can make identical commits fail before their gates run. Required-recipe Cargo tool installs must use `scripts/cargo-install-retry.sh` or a pinned prebuilt installer path so transient registry transport failures retry without weakening version, lockfile, or gate requirements. The shared `trunk-install` recipe owns Trunk `0.21.14`; UI recipes must depend on it, and harness code must only consume the provisioned binary instead of installing Trunk ad hoc.
...
- Justfile recipes must preserve the caller-selected PATH instead of invoking login shell startup files. Local NVM-managed Node versions and CI-provisioned native tools are part of the gate environment and must not be shadowed by unrelated user-profile toolchains.
- The `revaer-torrent-libt` build script must select one coherent libtorrent source per build: bundled override, explicit include-plus-library override, pkg-config, or a single fallback prefix. Do not mix broad Homebrew/system fallback include or library paths with pkg-config results because ABI-version mismatches can pass compilation and fail later at link or runtime.
- `just test-native` intentionally runs the `revaer-torrent-libt` native test suite with `--test-threads=1`. Keep all native tests enabled, but do not reintroduce parallel native libtorrent sessions without task-recorded evidence that the linked libtorrent versions and hosted runners are stable under that overlap.
