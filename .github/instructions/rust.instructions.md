...
- `just test`, `just test-features-min`, `just test-native`, `just db-init`, `just cov`, and `just validate` default `REVAER_TEST_DATABASE_URL` through `scripts/local-postgres-url.sh` or explicitly supplied environment values. `just test`, `just test-features-min`, and `just cov` default `RUST_TEST_THREADS=1` so schema-heavy disposable databases and filesystem race fixtures remain deterministic while still executing the complete test inventory; an explicit operator-provided value may increase concurrency for diagnosis. Do not document or reintroduce password-bearing database URI literals in Rust instructions, recipes, workflows, tests, or examples; local defaults may use non-production credentials only when the connection string is composed from variables or helper scripts. `just db-start` must derive both its connection URL and Docker `POSTGRES_USER`/`POSTGRES_PASSWORD` values from the same `REVAER_LOCAL_DB_*` environment contract, keep local endpoint normalization between `localhost` and `host.docker.internal`, use explicit `REVAER_DB_MANAGED=1` provenance when a caller synthesized a local `DATABASE_URL` and expects Docker lifecycle ownership, honor an already reachable explicit non-Docker `DATABASE_URL` when managed mode is not requested, reject managed mode for non-local endpoints, use portable TCP probes that do not require Python-only environments, support `REVAER_DB_DATA_DIR` for isolated local test data directories, provision local Docker Postgres containers with enough shared memory for schema-heavy test runs, wait for local containers to exit recovery before initialization, and retry transient Postgres recovery/startup/not-yet-accepting-connection errors around the checked-in initializer and explicit local database create/drop operations before treating the database as mismatched. Because `just` recipe lines do not share shell state, recipes that compute database URLs must pass `REVAER_TEST_DATABASE_URL`, `DATABASE_URL`, and any required managed-database provenance directly into every command that depends on them. When one recipe line chains database setup and tests, join dependent commands with `&&` so an earlier failure cannot be hidden by a later command. `just test-features-min` intentionally runs library, binary, and integration tests for `revaer-api` and `revaer-app` without invoking rustdoc; full workspace doctests remain covered by `just test`, while the minimal-feature gate stays focused on feature-configuration behavior. Keep recipes and docs aligned with that admin-connection workflow when test database bootstrapping changes.
...
- `just cov` records the workspace baseline with `cargo llvm-cov --workspace --all-features --no-report`, accumulates the ignored production media-runtime profile with `--no-clean` and matching all-feature/FFI instrumentation, then enforces the 90% per-package line threshold with `cargo llvm-cov report --package ...` against that shared dataset. Keep the coverage gate workspace-sourced so library crates receive credit for lines exercised by downstream crates and integration tests.
...
- `just test-media-conversion` must verify the prepared fixture catalog before running the ignored real-media integration suite and the production-adapter database runtime test; PR fixture jobs must invoke this canonical recipe and clean generated media under an always-run step.
